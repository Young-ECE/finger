...
class PwmControlProtocolTest(unittest.TestCase):
    def test_build_pwm_packet_matches_wire_format(self):
        from python_scripts.pwm_control_protocol import PACKET_LENGTH, build_pwm_packet
    ...
    def test_build_pwm_packet_rejects_non_integer_sequence_values(self):
        from python_scripts.pwm_control_protocol import build_pwm_packet

        for bad_value in (True, "1", 1.0):
            with self.subTest(bad_value=bad_value):
                with self.assertRaises(TypeError):
                    build_pwm_packet(sequence=bad_value, lcd1=0, led1=0, lcd2=0, led2=0)
